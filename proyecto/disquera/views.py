...
def contratos(request):
    ...
    if request.method == 'POST':
        ...
        '''sql = f"""SELECT c.id_contrato, c.fecha_inicio, c.fecha_fin, a.nombre_artista AS artista, s.nombre AS sede 
            ...
            with connections['america'].cursor() as cursor:
                cursor.execute(sql, values)
                america = cursor.fetchall()

            with connections['europa'].cursor() as cursor:
                cursor.execute(sql, values)
                america = cursor.fetchall()

            with connections['asia'].cursor() as cursor:
                cursor.execute(sql, values)
                america = cursor.fetchall()      
        '''
...
def regalias(request):
    ...
    if request.method == 'POST':
        ...
        '''sql = """SELECT r.id_contrato, tr.descripcion, r.monto
                ...
                WHERE r.id_contrato IN (SELECT id_contrato FROM contrato WHERE id_artista = %s) 
